    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/admin/blog" className="p-2 rounded-lg hover:bg-accent transition-colors text-muted-foreground">
      ...
      {preview ? (
        ...
        >
          <div className="flex flex-wrap gap-2 mb-4">
            {tags.map((tag) => (
              <span key={tag} className="text-xs px-3 py-1 rounded-full bg-primary text-primary-foreground" style={{ opacity: 0.8 }}>
                {tag}
              </span>
            ))}
          </div>
          ...
          {formData.excerpt && (
            <p className="text-muted-foreground text-lg mb-6 leading-relaxed border-l-4 pl-4" style={{ borderColor: "var(--color-primary)" }}>
              {formData.excerpt}
            </p>
          )}
          <div className="flex items-center gap-4 text-sm text-muted-foreground mb-8 pb-6 border-b" style={{ borderColor: "var(--color-border)" }}>
          ...
          <div>
            {formData.content ? (
              formData.content.split("\n").map((line, i) =>
                line.trim() ? (
                  <p key={i} className="text-muted-foreground leading-relaxed mb-4">{line}</p>
                ) : (
                  <br key={i} />
                )
              )
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="bg-background rounded-xl border p-6 space-y-4" style={{ borderColor: "var(--color-border)" }}>
            ...
            <div className="flex flex-wrap gap-2 p-2 rounded-lg border bg-muted/30" style={{ borderColor: "var(--color-border)" }}>
              {[
                { label: "H1", insert: "# " },
                { label: "H2", insert: "## " },
                { label: "H3", insert: "### " },
          ...
          <div className="bg-background rounded-xl border p-6 space-y-4" style={{ borderColor: "var(--color-border)" }}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {tagSuggestions.filter((t) => !tags.includes(t)).slice(0, 8).map((tag) => (
                  <button
                    key={tag}
                    onClick={() => addTag(tag)}
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1.5 rounded-lg text-sm">
                    {tag}
          ...
          <div className="bg-background rounded-xl border p-6 space-y-4" style={{ borderColor: "var(--color-border)" }}>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.key} className="flex items-center justify-between p-4 rounded-lg border" style={{ borderColor: "var(--color-border)" }}>
                  <div>
                  ...
                  <button
                    ...
                    style={{ backgroundColor: formData[item.key as keyof typeof formData] ? "var(--color-primary)" : "var(--color-muted)" }}
                  >
                    <span
                      className="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200"
                      style={{ transform: formData[item.key as keyof typeof formData] ? "translateX(20px)" : "translateX(0)" }}
                    />
                  </button>
                </div>
              ))}
